(ns platingcoord.actor
  "PlatingCoordActor — the ISCO-08 8122 metal finishing, plating and
  coating machine operators plant scheduling/logistics coordination
  actor as a `langgraph.graph/state-graph` (ADR-2607121000 / CLAUDE.md
  Actors section). One graph run = one plant-coordination operation
  ...
  cloud-itonami-isco-7535's tannerycoord.actor.
  ...
  The unconditional invariant: the Plating Plant Scheduling
  Coordination Advisor can never directly commit an operating record
  the PlatingCoordGovernor refuses, and can never make this actor
  finalize a plating-operation-execution decision (e.g. deciding to
  ...
  safe for handling), and can never override a plant safety officer's
  ...
  ONLY — it never operates plating equipment itself, and it never
...
(defn build-graph
  "Build a compiled PlatingCoordActor graph. `store` implements
  `platingcoord.store/Store`. `advisor` implements
  `platingcoord.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:plater-id (:plater-id request)
                                    :op (:op proposal)
